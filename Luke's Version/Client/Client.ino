...
String macAddress = "34:85:18:92:02:6c"; // <--- Slave MAC
...
void xorDecrypt(uint16_t data[2], uint16_t key) {
  for (int i = 0; i < 2; i++) {
    data[i] ^= key;
  }
}
...
void sendPoseEncrypted(){
  uint16_t x_u = xorCipher(x_to_u16_100(state_x), key);
  uint16_t y_u = xorCipher(x_to_u16_100(state_y), key);
  uint16_t t_u = xorCipher(theta_to_mrad_u16(state_theta), key);
  ...
  if (x && y && theta) {
    ...
    Serial.print("X STATUS = ");
    Serial.print(x);
    Serial.print(" Y STATUS = ");
    Serial.print(y);
    Serial.print(" THETA STATUS = ");
