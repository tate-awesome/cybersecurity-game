...
const float HEAD_OFFSET_M = 20.0f;     // Head position offset from vehicle center
const float Kps = 0.1f;                 // Speed proportional gain
const float Kp_rudder = 0.05f;          // Rudder proportional gain
const float SpeedMax = 50.0f;           // Maximum speed (m/s)
const float StopTol = 5.0f;             // Stop tolerance (meters)
...
void computeControlCommands(float target_x, float target_y, 
  ...
  float ThetaDes = atan2_approx(Yerr, Xerr);
