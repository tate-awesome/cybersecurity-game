...
#include <Wire.h>
#include <LiquidCrystal_I2C.h> //LiquidCrystalI2C library from "Frank de Brabander"
...
IPAddress serverIP(192, 168, 8, 137);  // <-- Server  IP
...
void setup() {
  ...
  Serial.println("[MASTER] Connecting to MQTT...");
  ...
  Serial.printf("[MASTER] MQTT Client ID: %s\n", clientId.c_str());
  
  if (mqttClient.connect(clientId.c_str())) {
    Serial.println("[MASTER] MQTT Connected!");
  } else {
    Serial.printf("[MASTER] MQTT Failed! State: %d\n", mqttClient.state());
    Serial.println("[MASTER] Continuing anyway...");
  }
}
...
void loop() {
  ...
  if (!mqttClient.connected()) {
    ...
    if (millis() - lastMqttReconnect > 5000) {
      ...
      Serial.println("[MASTER] Reconnecting MQTT...");
      Serial.printf("[MASTER] Client ID: %s, Broker: %s\n", clientId.c_str(), mqtt_server);
      
      if (mqttClient.connect(clientId.c_str())) {
        Serial.println("[MASTER] MQTT Reconnected!");
      } else {
        Serial.printf("[MASTER] MQTT Reconnect failed! State: %d\n", mqttClient.state());
  ...
  // Reconnect Modbus if disconnected
  if (!mb.isConnected(serverIP)) {
  ...
  // ← MOVE MQTT PUBLISHING HERE - Outside all if blocks!
  // Publish to MQTT every 100ms (independent of vehicle motion)
  static uint32_t lastMqttPub = 0;
  if (millis() - lastMqttPub > 100) {
    lastMqttPub = millis();
    
    if (mqttClient.connected()) {
      char msg[32];
      
      snprintf(msg, 32, "%.3f", state_x);
      mqttClient.publish("vessel/state/x", msg);
      
      snprintf(msg, 32, "%.3f", state_y);
      mqttClient.publish("vessel/state/y", msg);
      
      snprintf(msg, 32, "%.4f", state_theta);
      mqttClient.publish("vessel/state/theta", msg);
      
      Serial.printf("[MASTER] Published MQTT: X=%.1f Y=%.1f Theta=%.2f\n",
                    state_x, state_y, state_theta);
    } else {
      Serial.println("[MASTER] MQTT not connected, skipping publish");
    }
  }
