...
void loop() {
  ...
  // Add to Master_PWM.ino constants
const uint16_t HREG_PING = 20; 

// Add inside Master loop()
static uint32_t lastPingTime = 0;
if (millis() - lastPingTime > 500) { // Test every 500ms
    uint32_t startTime = millis();
    uint16_t pingVal = (uint16_t)(startTime & 0xFFFF); // Use lower 16 bits

    // Write ping to Slave
    if (mb.writeHreg(serverIP, HREG_PING, pingVal)) {
        uint16_t response;
        // Poll for response (you may need to wait a few task cycles)
        if (readH(HREG_PING, &response, 1)) {
            if (response == pingVal) {
                uint32_t latency = millis() - startTime;
                Serial.printf("[BENCHMARK] Modbus Round Trip: %u ms\n", latency);
            }
        }
    }
    lastPingTime = millis();
