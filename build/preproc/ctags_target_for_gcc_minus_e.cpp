...
void recordHvacSample(float room, float target) {
  ...
  g_hvac_history_head = (g_hvac_history_head + 1) % 40;
  if (g_hvac_history_count < 40) g_hvac_history_count++;
}
...
void setup() {
  ...
  setupRoutes();
  server.begin();
  Serial0.println("[AP] Web server started on http://192.168.4.1");
}
...
void loadPreferences() {
  ...
  Serial0.printf("[AP] Loaded NVS — SSID: '%s'  Flask: '%s'\n",g_ssid.c_str(), g_flask_ip.c_str());
}
...
void savePreferences(const String& ssid, const String& pass, const String& flask_ip) {
  ...
  prefs.end();
  Serial0.println("[AP] Preferences saved to NVS.");
}
...
void startAP() {
  ...
  Serial0.printf("[AP] Soft-AP started: SSID='%s'  IP=%s\n", AP_SSID, WiFi.softAPIP().toString().c_str());

  //captive portal
  dnsServer.start(DNS_PORT, "*", AP_IP);
  Serial0.println("[AP] Captive portal DNS started.");
}
...
void setupRoutes() {
  ...
  server.on("/hvac_history", HTTP_GET, [](AsyncWebServerRequest* req) {
    ...
    int start = (g_hvac_history_head - g_hvac_history_count + 40) % 40;
  ...
  server.on("/set_encryption", HTTP_POST,
    ...
    [](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t, size_t) {
      ...
      Serial0.printf("[AP] Encryption set: %s  key=%s\n",
  ...
  server.on("/set_filter_correction", HTTP_POST,
    ...
    [](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t, size_t) {
      ...
      Serial0.printf("[AP] filterCorrection set: %s\n",
  ...
  server.on("/set_target", HTTP_POST,
    ...
    [](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t, size_t) {
      ...
      Serial0.printf("[AP] Target set: (%.1f, %.1f)\n",
  ...
  server.on("/set_hvac_target", HTTP_POST,
    ...
    [](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t, size_t) {
      ...
      if (!err && doc.containsKey("target_temp")) {
        ...
        Serial0.printf("[AP] New HVAC target received: %.2f°F\n", g_target_temp);
        recordHvacSample(g_current_room_temp, g_target_temp);
        req->send(200, "application/json", "{\"status\":\"ok\"}");
      } else {
  ...
  server.on("/clear", HTTP_POST, [](AsyncWebServerRequest* req) {
      ...
      g_server_arr = g_server_doc.to<JsonArray>();
      g_devices.clear();
      Serial0.println("[AP] Telemetry data cleared.");
