    ...
    def is_running(self) -> bool:
        ...
        if self.is_connected and self.target_network is not None and not self._connecting:
            ...
            if current is not None and current == self.previous_network:
                self.buffer.put("wifi", f"NetworkManager already reconnected to '{current}' on its own.")
            elif self._last_deactivation_reason == NM.ActiveConnectionStateReason.USER_DISCONNECTED:
                # A real reason from NM, not a guess - the user (or some
                # other app) explicitly disconnected it, so say that
                # plainly instead of the misleading "may be out of range".
                self.buffer.put("wifi", f"'{self.target_network}' was manually disconnected.")
            else:
                self.buffer.put("wifi", f"Lost connection to '{self.target_network}'; it may no longer be in range.")
